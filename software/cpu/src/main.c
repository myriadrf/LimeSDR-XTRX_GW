...
int main()
{
	...
	XGpio_Initialize(&xtrx_ctrl, XPAR_XTRX_CTRL_GPIO_DEVICE_ID);



	XGpio_DiscreteWrite(&xtrx_ctrl, 1, 0x2);
...
//    LP8758_WR_REG(XPAR_I2C_CORES_I2C1_BASEADDR,0x09,0xD2);
	...
	LP8758_WR_REG(XPAR_I2C_CORES_I2C2_BASEADDR, 0x0A, 0xBE);
	...
	LP8758_WR_REG(XPAR_I2C_CORES_I2C2_BASEADDR, 0x10, 0xA2);
	LP8758_WR_REG(XPAR_I2C_CORES_I2C2_BASEADDR, 0x09, 0xD2);

	uint8_t regvals2[1];
    for (uint8_t i = 1; i<=35; i++)
    {
//    	LP8758_RD_REG(XPAR_I2C_CORES_I2C1_BASEADDR,i,&regvals1[i]);
    	LP8758_RD_REG(XPAR_I2C_CORES_I2C2_BASEADDR,i,&regvals2[0]);
    }

//	usleep(1000000);
	//Add I2C core resets. This seems to fix some weird issues
//	XIic_WriteReg(XPAR_I2C_CORES_I2C1_BASEADDR, XIIC_RESETR_OFFSET, XIIC_RESET_MASK);
//	XIic_WriteReg(XPAR_I2C_CORES_I2C2_BASEADDR, XIIC_RESETR_OFFSET, XIIC_RESET_MASK);
//	usleep(1000000);
	uint8_t i2c_buf[3];
	// Write config to DAC
	i2c_buf[0] = 0x04; // cmd
	...
	XGpio_DiscreteWrite(&xtrx_ctrl, 1, 0x7);
	...
	XIic_Recv(XPAR_I2C_CORES_I2C1_BASEADDR,I2C_TERMO_ADDR,i2c_buf,3,XIIC_STOP);
	...
	//	{
	//		//    	LP8758_RD_REG(XPAR_I2C_CORES_I2C1_BASEADDR,i,&regvals1[i]);
	//		LP8758_RD_REG(XPAR_I2C_CORES_I2C2_BASEADDR, i, &regvals2[i]);
	//	}
		...
		if (!(spirez & 0x01))
		{
			...
			case CMD_ANALOG_VAL_RD:
				...
				for (block = 0; block < LMS_Ctrl_Packet_Rx->Header.Data_blocks; block++)
				{
					...
					case 0:				   // dac val
						i2c_buf[0] = 0x08; // addr
						XIic_Send(XPAR_I2C_CORES_I2C1_BASEADDR, I2C_DAC_ADDR, i2c_buf, 1, XIIC_REPEATED_START);
